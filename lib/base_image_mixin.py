...
class BaseImageMixin(object):
    ...
    def build_image(self, frame, shot_state):
        image, shot_state = self.frame_to_image(frame, shot_state)
    ...
    def _frame_to_image(self, frame, pil_format, shot_state):
        av_format = AV2PIL_FORMAT_DICT.get(pil_format, 'rgb24')
        size, shot_state = self.optimize_size(frame, shot_state)
    ...
    def optimize_size(self, frame, shot_state):
        '''
    ...
    def __optimize_size(self, frame, shot_state):
        '''
            Resize frame before converting to PIL.Image.
